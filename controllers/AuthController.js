...
export const AuthController = {
  createAccount: async (req, res) => {
    ...
    if (email && password && name && username && birthday && sex) {
      try {
        ...
        return res
          .status(500)
          .json(ResponseDetail(500, { message: "Internal Server Error" }));
      }
    } else {
      return res
        .status(400)
        .json(ResponseDetail(400, { message: "Missing required fields." }));
    }
  },
    ...
    try {
      ...
      if (isMatch) {
        ...
        if (user) {
          ...
          return res
            .status(404)
            .json(ResponseDetail(404, { message: "User not found." }));
        }
      } else {
        return res
          .status(400)
          .json(
            ResponseDetail(400, { message: "Invalid verification token." })
          );
      }
    } catch (error) {
      console.log(error);
  ...
  refreshToken: async (req, res) => {
    ...
    if (!refreshToken1) {
      return res
        .status(401)
        .json(ResponseDetail(401, { message: "Vui lòng đăng nhập" }));
    }
    ...
    try {
      ...
      return res
        .status(500)
        .json(ResponseDetail(500, { message: "Lỗi máy chủ" }));
    }
  },
