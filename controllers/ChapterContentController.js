...
export const ChapterContentController = {
    ...
    getAllContentByChapterID: async (req, res) => {
        try {
            const chapterID = req.params.chapterID
            const response = await ChapterContentService.getAllContentByChapterID(chapterID)
            if (response.length>0) return res.status(200).json(ResponseData(200, response))
            return res.status(400).json(ResponseDetail(400, { message: "Không tìm thấy chương" }))
        } catch (error) {
            console.log(error)
